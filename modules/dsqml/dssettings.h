...
public:
    ...
    template<class T,class V> std::optional<ValueWMeta<T>> originalValueWithMeta(const std::string& key,V&& overload){
        auto val = getNodeViewWithMeta(key);
        if(!val.has_value()){
            qDebug(settingsParser)<<"Failed to find value at key "<<key.c_str();
            return std::nullopt;
    ...
    template<class T> std::optional<ValueWMeta<T>> getWithProcess(const std::string& key,std::function<std::optional<ValueWMeta<T>>(toml::node_view<toml::node> node, toml::table* meta)> process){
        auto val = getNodeViewWithMeta(key);
