...
static const QLoggingCategory& lgExif() {
	static const QLoggingCategory category("exif");
	return category;
}
...
class ExifHelper {
  public:
	...
	static bool getImageSize(const std::string filePath, int& outWidth, int& outHeight) {
		...
		if (!fp) {
			qCWarning(lgExif) << "Can't open file for exif data. " << filePath;
			return false;
		}
		fseek(fp, 0, SEEK_END);
		...
		if (fread(buf, 1, fsize, fp) != fsize) {
			qCWarning(lgExif) << "Can't read file for exif data. " << filePath;
			delete[] buf;
			return false;
		}
		fclose(fp);
		...
		if (code) {
			if (code == 1983) {
				...
				qCWarning(lgExif) << "Error parsing EXIF: code " << code << " for filepath: " << filePath;
			}
			return false;
		}
