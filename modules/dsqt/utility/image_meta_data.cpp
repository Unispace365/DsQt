...
#include "utility/exif_reader.h"
#include "utility/file_meta_data.h"

Q_LOGGING_CATEGORY(lgImageMetaData, "image_meta_data")
namespace dsqt {
...
namespace {
	...
	bool get_format_jpg(const std::string& filename, glm::vec2& outSize) {
		std::ifstream file(filename, std::ios_base::binary | std::ios_base::in);
		if (!file.is_open() || !file) return false;
	...
	void super_slow_image_atts(const std::string& filename, glm::vec2& outSize, std::function<void(void)> callback = nullptr) {
		try {
			if (filename.empty()) return;

			// check to see if path exists
			if (dsqt::safeFileExistsCheck(filename)) {
				...
				qCWarning(lgImageMetaData)
					<< "ImageFileAtts Going to load image synchronously; this will affect performance, filename: " << filename;

				// auto s = ci::Surface8u(ci::loadImage(filename));
				auto s = QImage(QString::fromStdString(filename));
				if (!s.isNull()) {
					outSize = glm::vec2(static_cast<float>(s.width()), static_cast<float>(s.height()));
				} else {
					qCWarning(lgImageMetaData) << "super_slow_image_atts: file could not be loaded, filename: " << filename;
					outSize = glm::vec2();
				}
 ...
 * \class ImageMetaData
 */
ImageMetaData::ImageMetaData(QObject* parent) : QObject(parent), mSize(0.0f, 0.0f) {}
