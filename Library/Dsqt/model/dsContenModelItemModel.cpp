...
DsContenModelItemModel::DsContenModelItemModel(QObject* parent)
    ...
    connect(mEngine, &DsQmlApplicationEngine::rootUpdated, this, [this]() {
        if(m_rootItem){
...
QModelIndex DsContenModelItemModel::index(int row, int column, const QModelIndex &parent) const
{
    ...
    ContentModelItem *parentItem = parent.isValid() ? static_cast<ContentModelItem*>(parent.internalPointer()) : m_rootItem.get();
...
int DsContenModelItemModel::rowCount(const QModelIndex &parent) const
{
    ...
    const ContentModelItem *parentItem = parent.isValid()
                                     ? static_cast<const ContentModelItem*>(parent.internalPointer())
                                     : m_rootItem.get();
...
{
    if (parent.isValid())
        return static_cast<ContentModelItem*>(parent.internalPointer())->columnCount();
...
QHash<int, QByteArray> DsContenModelItemModel::roleNames() const {
    return QHash<int, QByteArray>{{Qt::DisplayRole, "display"}, {Qt::UserRole, "id"},{Qt::UserRole+1, "contentModel"}};
}
...
std::unique_ptr<ContentModelItem> DsContenModelItemModel::updateModelData(const ContentModelRef &model)
{
    ...
    for(auto& child: model.getChildren()){
        ...
        item->m_childItems.push_back(std::move(childItem));
    }
...
void DsContenModelItemModel::updateModelData()
{
    ...
    auto contentRoot =updateModelData(mEngine->getContentRoot());
    contentRoot->m_parentItem = m_rootItem.get();
    m_rootItem->m_childItems.push_back(std::move(contentRoot));
}
...
QVariant ContentModelItem::data(int role) const
{
    switch(role){
        case Qt::DisplayRole:
            return m_itemData.value("name", QString());
            break;
        case Qt::UserRole:
            return m_itemData.value("id", QString());
            break;
        case Qt::UserRole + 1:
            return m_itemData.value("model", QVariant());
            break;
        default:
...
int ContentModelItem::row() const
{
    ...
    const auto it = std::find_if(m_parentItem->m_childItems.cbegin(), m_parentItem->m_childItems.cend(),
                                 [this](const std::unique_ptr<ContentModelItem> &treeItem) {
