...
ContentProperty::ContentProperty(const QString& name, const QString& value, int valueInt, double valueDouble) {
    mName        = name;
    mValue       = value;
    mIntValue    = valueInt;
...
void ContentProperty::setValue(const QDate& value) {
    mValue       = value.toString("yyyy-MM-dd");
    mIntValue    = 0;
    mDoubleValue = 0.0;
}

void ContentProperty::setValue(const QTime& value) {
    mValue       = value.toString("HH:mm:ss");
    mIntValue    = 0;
    mDoubleValue = 0.0;
}
...
bool ContentProperty::operator==(const ContentProperty& b) const {
    ...
    // bool sameResource = (mResource.get() == b.mResource.get()) || (resourcesComparable && (*mResource ==
    // *b.mResource));
...
QDate ContentProperty::getDate() const {
    return QDate::fromString(mValue, "yyyy-MM-dd");
}

QTime ContentProperty::getTime() const {
...
ContentModelRef::ContentModelRef(const ContentModelRef& other)
    : mData(other.mData) {
}
...
bool ContentModelRef::empty() const {
    ...
    if (mData->mId.isEmpty() && mData->mName.isEmpty() && mData->mLabel.isEmpty() && mData->mUserData == nullptr &&
        mData->mChildren.empty() && mData->mProperties.empty() && mData->mReferences.empty() &&
...
bool ContentModelRef::equalChildrenAndReferences(
    const ContentModelRef& b, std::vector<std::pair<const void*, const void*>>& alreadyChecked) const {
    if (weakEqual(b)) {
        if (!mData) return true;
...
QDateTime ContentModelRef::getPropertyDateTime(const QString& propertyNameDate, const QString& propertyNameTime) const {
    return {getProperty(propertyNameDate).getDate(), getProperty(propertyNameTime).getTime()};
}

QDate ContentModelRef::getPropertyDate(const QString& propertyName) const {
    return getProperty(propertyName).getDate();
}

QTime ContentModelRef::getPropertyTime(const QString& propertyName) const {
    return getProperty(propertyName).getTime();
}
...
ContentModelRef ContentModelRef::getDescendant(const QString& childId) const {
...
ContentModelRef ContentModelRef::findChildByPropertyValue(const QString& propertyName,
...
const std::unordered_map<QString, dsqt::model::ContentModelRef>&
ContentModelRef::getReferences(const QString& name) const {
...
ContentModelRef ContentModelRef::getReference(const QString& referenceName, const QString nodeId) const {
...
void ContentModelRef::printTree(const bool verbose, const QString& indent) const {
    if (empty() || !mData) {
        ...
        qWarning() << indent << "ContentModel id:" << mData->mId << " name:" << mData->mName
                   << " label:" << mData->mLabel;
        if (verbose) {
