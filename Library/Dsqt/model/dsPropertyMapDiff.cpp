...
PropertyMapDiff::PropertyMapDiff(const DsQmlContentModel& from, const DsQmlContentModel& to) {
    m_visited.clear();
    diffMaps(from, to, {});
}
...
void PropertyMapDiff::diffMaps(const DsQmlContentModel& from, const DsQmlContentModel& to, const QStringList& path) {
    ...
    for (int i = 0; i < common; ++i) {
        ...
        auto* fMap = qobject_cast<DsQmlContentModel*>(fObj);
        auto* tMap = qobject_cast<DsQmlContentModel*>(tObj);
        if (fMap && tMap) {
    ...
    // RemovedNode
    DsQmlContentModel emptyMap(ContentModelRef(), nullptr);
    for (int i = common; i < fCount; ++i) {
        auto*  oldMap = qobject_cast<DsQmlContentModel*>(fromChildren[i].value<QObject*>());
        Change c;
        c.type     = RemovedNode;
    ...
    for (int i = common; i < tCount; ++i) {
        auto*  newMap = qobject_cast<DsQmlContentModel*>(toChildren[i].value<QObject*>());
        Change c;
        c.type     = AddedNode;
        c.path     = path + QStringList(QString::number(i));
...
DsQmlContentModel* PropertyMapDiff::nodeAtPath(DsQmlContentModel& root, const QStringList& path) const {
    DsQmlContentModel* current = &root;
    for (const QString& seg : path) {
        ...
        current      = qobject_cast<DsQmlContentModel*>(obj);
        if (!current) break;
    }
    return current;
}

const DsQmlContentModel* PropertyMapDiff::nodeAtPath(const DsQmlContentModel& root, const QStringList& path) const {
    // same as above, but const
    const DsQmlContentModel* current = &root;
    for (const QString& seg : path) {
        bool ok  = false;
        int  idx = seg.toInt(&ok);
        if (!ok) break;
...
void PropertyMapDiff::apply(DsQmlContentModel& map, ReferenceMap* itemMap) const {
    qDebug() << "Apply changes to qml model";
    for (const Change& c : m_changes) {
        DsQmlContentModel* node = nullptr;
        switch (c.type) {
        case AddedProperty:
        ...
        case AddedNode: {
            node         = qobject_cast<DsQmlContentModel*>(c.newValue.value<QObject*>());
        ...
        case RemovedNode: {
            node         = qobject_cast<DsQmlContentModel*>(c.oldValue.value<QObject*>());
            auto* parent = nodeAtPath(map, c.path.mid(0, c.path.size() - 1));
            if (!parent) break;
            QVariantList kids = parent->value(u"children"_s).toList();
            auto         id   = node->value("id").toString();
            ...
            itemMap->remove(id);
            int idx = c.path.last().toInt();
            if (idx >= 0 && idx < kids.size()) kids.removeAt(idx);
            parent->insert(u"children"_s, kids);
            emit parent->valueChanged(u"children"_s, kids);
            break;
        }
