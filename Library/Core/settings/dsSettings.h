...
using tomlType   = std::variant<ValueWMeta<toml::value<bool>>, ValueWMeta<toml::value<int64_t>>,
                                ValueWMeta<toml::value<double>>, ValueWMeta<toml::value<std::string>>,
                                ValueWMeta<toml::table>, ValueWMeta<toml::array>, ValueWMeta<toml::value<toml::date>>,
                                ValueWMeta<toml::value<toml::time>>, ValueWMeta<toml::value<toml::date_time>>>;
...
/// As settings are loaded into the collection the form a stack with the first file loaded forming the bottom of the
/// stack. when settings are recalled the object looks first at any setting that was set at runtime and then
/// systematically goes down the stack looking for the setting until it reached the first file loaded. If it fails to
/// find the setting it returns an error. You can use the static functions ::getSettings or ::getSettingsOrCreate to
/// find or create a collection. a new collection has no settings in it. settings can be added to a stack of settings by
/// calling ::loadSettingFile
///
/// todo
/// ----
...
/// The actuall replication of the DSCinder system is done by the DSEnvironment class. Eventually it will be able to
/// tell and editor where a setting comes from
///
...
class DsSettings : public QObject {
  ...
  public:
    ...
    static constexpr bool is_valid_setting_type =
        ...
        std::is_same_v<T, toml::array> || std::is_same_v<T, QMap<QString, QVariant>> || //
    ...
    void setDateFormat(Qt::DateFormat format, bool clearCustom = true) {
        ...
        if (clearCustom && !mCustomDateFormat.isEmpty()) {
            qWarning(lgSettingsParser) << "setDateFormat is clearing a custom date format for "
                                       << QString::fromStdString(mName) << " settings.";
            mCustomDateFormat = "";
        }
    }
    ...
    void setCustomDateFormat(QString format) { mCustomDateFormat = format; }
    ...
    template <>
    std::optional<QVariantList> get(const std::string& key) {
    ...
    template <>
    std::optional<QVariantMap> get(const std::string& key) {
    ...
    template <>
    std::optional<ValueWMeta<QVector<QVariant>>> getWithMeta(const std::string& key);
    template <>
    std::optional<ValueWMeta<QMap<QString, QVariant>>> getWithMeta(const std::string& key);
...
*/
  ...
  private:
    ...
    static std::unordered_map<
        std::string, std::function<void(QColor& resultcolor, float v1, float v2, float v3, float v4, float v5)>>
