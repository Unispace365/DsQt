...
class DsSpoutMaterial;

/**
 * DsSpoutTextureNode - Scene graph node for rendering Spout textures
 * Uses Qt RHI for cross-platform texture rendering
 */
class DsSpoutTextureNode : public QSGImageNode
{
public:
    explicit DsSpoutTextureNode(QQuickWindow* window);
    ~DsSpoutTextureNode() override;
...
private:
    ...
    DsSpoutMaterial* m_material = nullptr;
