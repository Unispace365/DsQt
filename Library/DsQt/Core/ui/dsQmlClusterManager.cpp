...
namespace dsqt::ui {
DsQmlClusterManager::DsQmlClusterManager(QQuickItem* parent)
    : QQuickItem(parent) {
...
void DsQmlClusterManager::parseTouch(const TouchInfo& ti) {
    ...
    if (ti.mState == QEventPoint::Pressed) {
        ...
        if (relatedCluster == nullptr && alreadyCluster == nullptr) {
            ...
            emit clusterUpdated(
                QEventPoint::Updated,
                clusty); // if (mClusterUpdateFunction) mClusterUpdateFunction(ds::ui::TouchInfo::Moved, clusty);
        }
        ...
        emit clusterUpdated(
            QEventPoint::Updated,
            clustyMcClustClust); // if (mClusterUpdateFunction) mClusterUpdateFunction(ds::ui::TouchInfo::Moved,
                                 // clustyMcClustClust);
        ...
        if (clustyMcClustClust->mTriggerable) {
            ...
            if (clustyMcClustClust->mBoundingBox.width() < m_boundingBoxSize &&
                ...
                (QDateTime::currentMSecsSinceEpoch() - clustyMcClustClust->mInitialTouchTime) > m_triggerTime * 1000) {
        ...
        if (clustyMcClustClust->mTouches.size() == 0) {
            ...
            emit clusterUpdated(
                ...
                clustyMcClustClust); // if (mClusterUpdateFunction) mClusterUpdateFunction(ds::ui::TouchInfo::Removed,
                                     // clustyMcClustClust);
...
void DsQmlClusterManager::setMinClusterTouchCount(int newClusterTouchCount) {
    if (m_minClusterTouchCount == newClusterTouchCount) return;
...
void DsQmlClusterManager::setHoldOpenOnTouch(bool newHoldOpenOnTouch) {
    if (m_holdOpenOnTouch == newHoldOpenOnTouch) return;
...
void DsQmlClusterManager::setDoubleTapActivation(bool newDoubleTapActivation) {
    if (m_doubleTapActivation == newDoubleTapActivation) return;
...
void DsQmlClusterManager::setBoundingBoxSize(float newBoundingBoxSize) {
    if (qFuzzyCompare(m_boundingBoxSize, newBoundingBoxSize)) return;
...
void DsQmlClusterManager::setMinClusterSeperation(float newMinClusterSeperation) {
    if (qFuzzyCompare(m_minClusterSeparation, newMinClusterSeperation)) return;
...
void DsQmlClusterManager::setTriggerTime(float newTriggerTime) {
    if (qFuzzyCompare(m_triggerTime, newTriggerTime)) return;
