...
DsResource::Id::Id(const valType__ value)
    : mType(CMS_TYPE)
    , mValue(value) {
}

DsResource::Id::Id(const QString value)
    : mType(CMS_TYPE)
    , mValue(qHashHalf(value)) {
}

DsResource::Id::Id(const char type, const valType__ value)
    : mType(type)
    , mValue(value) {
}

DsResource::Id::Id(const char type, const QString value)
    : mType(type)
    , mValue(qHashHalf(value)) {
}
...
DsResource::DsResource(const DsResource::Id dbid, const int type, const double duration, const float width,
                       const float height, const QString filename, const QString path, const int thumbnailId,
