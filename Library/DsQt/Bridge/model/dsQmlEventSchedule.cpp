...
DsQmlEventSchedule::DsQmlEventSchedule(const QString& type_name, QObject* parent)
    ...
    connect(&bridge, &bridge::DsBridge::databaseChanged, this, &DsQmlEventSchedule::updateNow);
...
bool DsQmlEventSchedule::isEventNow(const bridge::DatabaseRecord& event, QDateTime localDateTime) {
    ...
    if (!startTime.isValid()) {
        qCWarning(lgQmlEvent) << "Couldn't parse the start time for an event of type "
                            << event.value("type_key").toString();
        qCWarning(lgQmlEventVerbose) << "Start Time of event: " << event.value(StartTime).toString();
        return false;
    }
    if (!endTime.isValid()) {
        qCWarning(lgQmlEvent) << "Couldn't parse the end time for an event of type "
                            << event.value("type_key").toString();
        qCWarning(lgQmlEventVerbose) << "End Time of event: " << event.value(EndTime).toString();
        return false;
    }
    if (localDateTime.time() < startTime || localDateTime.time() >= endTime) {
        qCDebug(lgQmlEventVerbose) << "Event happens outside the current time: " << event.value("record_name").toString()
...
bool DsQmlEventSchedule::isEventToday(const bridge::DatabaseRecord& event, QDate localDate) {
    ...
    if (!startDate.isValid()) {
        qCWarning(lgQmlEvent) << "Couldn't parse the start date for an event of type "
                            << event.value("type_key").toString();
        qCWarning(lgQmlEventVerbose) << "Start Date of event: " << event.value(StartDate).toString();
        return false;
    }
    if (!endDate.isValid()) {
        qCWarning(lgQmlEvent) << "Couldn't parse the end date for an event of type "
                            << event.value("type_key").toString();
        qCWarning(lgQmlEventVerbose) << "End Date of event:" << event.value(EndDate).toString();
        return false;
    }

    if (localDate < startDate || localDate > endDate) {
        qCWarning(lgQmlEventVerbose) << "Event happens outside the current date: "
    ...
    qCDebug(lgQmlEventVerbose) << "Event not scheduled for the current weekday: " << event.value("record_name").toString()
                             << "(" << event.value("uid").toString() << ")";
    return false;
}
...
void DsQmlEventSchedule::update(const QDateTime& localDateTime) {
    ...
    QFuture<QList<DsQmlEvent*>> future = QtConcurrent::run([=]() {
        ...
        const auto& content = bridge.database();
